...
/* RX8025T */
#define RX8025T_REG_EXTREG      0x0D
#define RX8025T_REG_FLAG        0x0E
#define RX8025T_BIT_EXTREG_TEST (1 << 7)
#define RX8025T_BIT_FLAG_VLF    (1 << 1)
#define RX8025T_BIT_FLAG_VDET   (1 << 0)

static unsigned int isRX8025T = 0;
...
static int rx8025_read_reg(struct i2c_client *client, int number, u8 *value)
{
        ...
        if (isRX8025T)
	    ret = i2c_smbus_read_byte_data(client, (number & 0x0F) | 0x00);
        else
            ret = i2c_smbus_read_byte_data(client, (number << 4) | 0x08);
			    ...
			    int number, u8 length, u8 *values)
{
        ...
        if (isRX8025T)
            ret = i2c_smbus_read_i2c_block_data(client, (number & 0x0F) | 0x00,
                                                length, values);
        else
	    ret = i2c_smbus_read_i2c_block_data(client, (number << 4) | 0x08,
						length, values);
...
static int rx8025_write_reg(struct i2c_client *client, int number, u8 value)
{
        ...
        if (isRX8025T)
            ret = i2c_smbus_write_byte_data(client, number & 0x0F, value);
        else
	    ret = i2c_smbus_write_byte_data(client, number << 4, value);
			     ...
			     int number, u8 length, u8 *values)
{
        ...
        if (isRX8025T)
            ret = i2c_smbus_write_i2c_block_data(client, (number & 0x0F) | 0x00,
                                                 length, values);
        else
	    ret = i2c_smbus_write_i2c_block_data(client, (number << 4) | 0x08,
						 length, values);
...
static int rx8025_init_client(struct i2c_client *client, int *need_reset)
{
	...
	int err;
        ...
        if (isRX8025T) {
            err = rx8025_read_regs(rx8025->client, RX8025T_REG_FLAG, 2, ctrl);
            if (err)
                goto out;

            if (ctrl[0] & RX8025T_BIT_FLAG_VLF) {
                dev_warn(&client->dev, "power-on reset was detected, "
                         "you may have to readjust the clock\n");
                *need_reset = 1;
            }

            if (ctrl[0] & RX8025T_BIT_FLAG_VDET) {
            ...
            if (*need_reset || need_clear) {
                err = rx8025_write_reg(client, RX8025T_REG_FLAG, 0x00);
            }
        }
        else {
...
static int rx8025_read_alarm(struct device *dev, struct rtc_wkalrm *t)
{
	...
	int err;
        ...
        if (isRX8025T) {
            dev_warn(&client->dev, "rx8025_read_alarm: unsupport for RX8025T\n");
            return -EINVAL;
        }
...
static int rx8025_set_alarm(struct device *dev, struct rtc_wkalrm *t)
{
	...
	int err;
        ...
        if (isRX8025T) {
            dev_warn(&client->dev, "%s: unsupport for RX8025T\n", __func__);
            return -EINVAL;
        }
...
static int rx8025_alarm_irq_enable(struct device *dev, unsigned int enabled)
{
	...
	int err;
        ...
        if (isRX8025T) {
            dev_warn(&rx8025->client->dev, "%s: unsupport for RX8025T\n", __func__);
            return -EINVAL;
        }
...
static void rtcType_check(struct i2c_client *client)
{
    u8 pre_value = 0x00, write_value = 0x00, read_value = 0x00;
    u8 addr = 0x0E;
    u8 mask = 1<<4;

    rx8025_read_reg(client, addr, &pre_value);

    write_value = (pre_value & ~mask) | (~pre_value & mask);
    rx8025_write_reg(client, addr, write_value);

    rx8025_read_reg(client, addr, &read_value);

    isRX8025T = (write_value != read_value);

    if (isRX8025T)
        dev_info(&client->dev, "RX8025T enabled\n");
    else
        rx8025_write_reg(client, addr, pre_value);  /* restore */
}
