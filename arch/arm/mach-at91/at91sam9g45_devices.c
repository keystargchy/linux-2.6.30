...
static const unsigned spi0_standard_cs[4] = { AT91_PIN_PB3, AT91_PIN_PD24, AT91_PIN_PB19, AT91_PIN_PD27 };
 ...
 * 1 1  : 10.4
 */
int __init setup_panel_config(void)
{
    int val = 0;

    at91_set_gpio_input(AT91_PIN_PD29, 0);
    at91_set_gpio_input(AT91_PIN_PD30, 0);

    val = (at91_get_gpio_value(AT91_PIN_PD30) << 1) | \
          (at91_get_gpio_value(AT91_PIN_PD29));
    ...
    switch (val) {
        ...
        break;
        default:
            printk ("Warning: Invalid lcd config[%02x]. Set default\n", val);
            val = 0x00;
        break;
    }
    return val;
...
void __init at91_add_device_lcdc(struct atmel_lcdfb_info *data)
{
    ...
    if (setup_panel_config() >= 0x03) {     /* 10.4 or above */
	    at91_set_A_periph(AT91_PIN_PE3, 0);	/* LCDVSYNC */
    	at91_set_A_periph(AT91_PIN_PE4, 0);	/* LCDHSYNC */
    }
    else {
        at91_set_gpio_input(AT91_PIN_PE3, 0);
        at91_set_gpio_input(AT91_PIN_PE4, 0);
    }
...
void __init at91_add_device_pwm(u32 mask)
{
	...
	if (mask & (1 << AT91_PWM1))    /* used as backlight pwm */
...
static inline void configure_usart0_pins(unsigned pins)
{
	...
	if (pins & ATMEL_UART_RTS)
		at91_set_B_periph(AT91_PIN_PB17, 1);	/* RTS0 */
	if (pins & ATMEL_UART_CTS)
		at91_set_B_periph(AT91_PIN_PB15, 1);	/* CTS0 */
}
...
static inline void configure_usart1_pins(unsigned pins)
{
	...
	if (pins & ATMEL_UART_RTS)
		at91_set_A_periph(AT91_PIN_PD16, 1);	/* RTS1 */
	if (pins & ATMEL_UART_CTS)
		at91_set_A_periph(AT91_PIN_PD17, 1);	/* CTS1 */
}
...
static inline void configure_usart2_pins(unsigned pins)
{
	...
	if (pins & ATMEL_UART_RTS)
		at91_set_B_periph(AT91_PIN_PC9, 1);	/* RTS2 */
	if (pins & ATMEL_UART_CTS)
		at91_set_B_periph(AT91_PIN_PC11, 1);	/* CTS2 */
}
...
static inline void configure_usart3_pins(unsigned pins)
{
	...
	if (pins & ATMEL_UART_RTS)
		at91_set_B_periph(AT91_PIN_PA23, 1);	/* RTS3 */
	if (pins & ATMEL_UART_CTS)
		at91_set_B_periph(AT91_PIN_PA24, 1);	/* CTS3 */
}
